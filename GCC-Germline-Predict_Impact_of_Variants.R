...
imageFile = file.path(rdataDir, sprintf("GCC-Germline-Predict_Impact_of_Variants-%s.R", canType))
load(imageFile)
...
if (file.exists(cancerGenesFile)) {
    ...
    ## Update gene symbols of cancer genes
    cancerGenesGrpModDf = cancerGenesGrpDf
    cancerGenesGrpModDf$gene = as.character(cancerGenesGrpModDf$gene)
    cancerGenesGrpModDf$dbs = as.character(cancerGenesGrpModDf$db)
    for (i in 1:nrow(cancerGenesGrpModDf)) {
        gene = cancerGenesGrpModDf[i, "gene"]
        geneInfoExtSymbolDf = subset(geneInfoExtDf, Symbol == gene)
        geneInfoExtSynonymDf = subset(geneInfoExtDf, Synonym == gene)
        if (nrow(geneInfoExtSymbolDf) == 0 & nrow(geneInfoExtSynonymDf) > 0) {
            ngene = geneInfoExtSynonymDf[1, "Symbol"]
            cancerGenesGrpModDf[i, "gene"] = ngene
            cat(sprintf("%d: %s -> %s\n", i, gene, ngene))
        }
    }
    write.table(cancerGenesGrpModDf, cancerGenesFile, sep = "\t", row.names = F, col.names = T, quote = F)
}
...
geneResLog2ExpFile = file.path(tcgaCanDir, sprintf("Residual_Log2_Gene_Expression-Tumor-EUR-%s.new.txt", canType))
geneResLog2ExpDf = read.delim(geneResLog2ExpFile, header = T, as.is = T)


##
## Read integered genotype data
##
#genoIntFile = file.path(eqtlCanDir, "Genotypes.txt")
#genoIntDf = read.delim(genoIntFile, header = T, as.is = T)
...
##
#vcfFiles = mixedsort(Sys.glob(file.path(germDir, sprintf("VCF/CANCERS/%s/*/chrs/*/*rare.vcf.bgz", canType))))
novelVcfFiles = mixedsort(Sys.glob(file.path(germDir, sprintf("VCF/CANCERS/%s/*/chrs/*/*novel.vcf.bgz", canType))))
#vcfFiles = c(vcfFiles, novelVcfFiles)
vcfFiles = c(novelVcfFiles)
...
for (i in 1:length(vcfFiles)) {
    ...
    csqColNames = strsplit(gsub(".*(Allele.*)$", "\\1", info(header(vcf))["CSQ",]$Description), "|", fixed = T)[[1]]

    # Expand VEP annotation (CSQ)
    pb <- txtProgressBar(min = 0, max = nrow(vcf), width = 80, style = 3)
    csqExpVarsDf = data.frame()
    for (j in 1:nrow(vcf)) {
        setTxtProgressBar(pb, j)
        v = vcf[j]
        rowDataDf = as.data.frame(rowData(v))
        infoVcfNoCsq = info(v)[, -which(colnames(info(v)) == "CSQ")]
        csqs = unlist(info(v)$CSQ)
        for (k in 1:length(csqs)) {
            csq = csqs[k]
            csqCols = strsplit(csq, "|", fixed = T)[[1]]
            if (length(csqCols) < 47) {
                csqCols = c(csqCols, rep("", 47-length(csqCols)))
            }
            names(csqCols) = csqColNames
            csqExpVarsDf = rbind(csqExpVarsDf, cbind(data.frame(id = rownames(rowDataDf),
                                                                rowDataDf[, c("seqnames", "start", "end", "REF", "ALT", "QUAL")],
                                                                stringsAsFactors = FALSE),
                                                     as.data.frame(infoVcfNoCsq, stringsAsFactors = FALSE), t(csqCols)))
        }
    }
    close(pb)
    ...
    # Test for 
    # * Survival analysis (bp level)
    # * Mutual exclusivity (gene level?)
    # * Expression changes (gene level)
    #
    ...
    # * TCGA AWG SMG
    # * allOnco
    for (j in 1:nrow(csqExpVarsDf)) {
        p = csqExpVarsDf[j,]
        gts = geno(vcf)$GT[p$id,]
        mutPatientIds = as.vector(sapply(grep("TCGA", names(gts)[gts != "." & gts != "0/0"], value = T),
                               function(x) { paste(strsplit(x, "-", fixed = T)[[1]][1:3], collapse = "_") }))
        wdtPatientIds = as.vector(sapply(grep("TCGA", names(gts)[gts != "." & gts == "0/0"], value = T),
                               function(x) { paste(strsplit(x, "-", fixed = T)[[1]][1:3], collapse = "_") }))
        if (elementLengths(p$distalDhsToPromoterDhs) > 0) {
            print(j)
            geneNames = unlist(p$distalDhsToPromoterDhs)
            for (geneName in geneNames) {
                #mutExpDf = geneExpMergedDf[geneName, intersect(colnames(geneExpMergedDf),mutPatientIds)]
                #wdtExpDf = geneExpMergedDf[geneName, intersect(colnames(geneExpMergedDf),wdtPatientIds)]
                mutExpDf = geneExpMergedDf[geneName, mutPatientIds]
                wdtExpDf = geneExpMergedDf[geneName, wdtPatientIds]
                wtPvalueGeneExpMutVsWdt = my.wilcox.test.p.value(as.numeric(mutExpDf), as.numeric(wdtExpDf))
                ttPvalueGeneExpMutVsWdt = my.t.test.p.value(as.numeric(mutExpDf), as.numeric(wdtExpDf))
                if (is.na(wtPvalueGeneExpMutVsWdt)) next
                if (wtPvalueGeneExpMutVsWdt < 0.05) print(geneName)
            }
        }
    ...
    # Novel variants
    novelVcf = vcf[unlist(is.na(info(vcf)$TG_ALL_AF) &
                          is.na(info(vcf)$TA_AF) &
                          (info(vcf)$EUR_AF >= 0.05 & info(vcf)$EUR_AF <= 0.95) &
                          info(vcf)$DBSNP == "." &
    ...
    for (j in 1:nrow(novelVcf)) {
        ...
        for (k in 1:length(csqs)) {
            ...
            csqExpVarsDf = rbind(csqExpVarsDf, cbind(data.frame(id = rownames(rowDataDf),
                                                          rowDataDf[, c("seqnames", "start", "end", "REF", "ALT", "QUAL")],
                                                          stringsAsFactors = FALSE),
                                               as.data.frame(infoVcfNoCsq), t(csqCols)))
        }
    }



    geneResLog2ExpFile = file.path("/home/sl279/BiO/Research/GCC/Germline/TCGA/CRC/Residual_Log2_Gene_Expression-Tumor-EUR-CRC.txt")
    geneResLog2ExpDf = read.delim(geneResLog2ExpFile, header = T, as.is = T)

    s1pr1ExpDf = geneExpMergedDf[unlist(info(regVcf[1])$fantom5EnhancerTssAssociations),]
    genosFile = file.path("/home/sl279/BiO/Research/GCC/Germline/EQTL/CRC.old/Genotypes.txt")
    genosDf = read.delim(genosFile, header = T, as.is = T)
    s1pr1GenDf = genosDf[genosDf$id == "1:101774797_G/A",]
    mutPatientIds = grep("TCGA", colnames(s1pr1GenDf)[s1pr1GenDf > 0], value = T)
    wdtPatientIds = grep("TCGA", colnames(s1pr1GenDf)[s1pr1GenDf == 0], value = T)
    #mutExpDf = geneExpMergedDf["S1PR1", mutPatientIds]
    #wdtExpDf = geneExpMergedDf["S1PR1", wdtPatientIds]
    mutExpDf = geneResLog2ExpDf[geneResLog2ExpDf$id == "S1PR1", mutPatientIds]
    wdtExpDf = geneResLog2ExpDf[geneResLog2ExpDf$id == "S1PR1", wdtPatientIds]

    wtPvalueGeneExpMutVsWdt = my.wilcox.test.p.value(as.numeric(mutExpDf), as.numeric(wdtExpDf))
    ttPvalueGeneExpMutVsWdt = my.t.test.p.value(as.numeric(mutExpDf), as.numeric(wdtExpDf))

    # Common variants
    commonVcf = vcf[unlist(!is.na(info(vcf)$TG_ALL_AF) &
                           info(vcf)$TG_ALL_AF >= 0.01 &
                           info(vcf)$TG_EAS_AF >= 0.01 &
                           info(vcf)$TG_EUR_AF >= 0.01 &
                           info(vcf)$TG_AFR_AF >= 0.01 &
                           info(vcf)$TG_AMR_AF >= 0.01 &
                           info(vcf)$TG_SAS_AF >= 0.01)]

    nrow(commonVcf)

    info(commonVcf)[grepl("splice_donor_variant", lapply(info(commonVcf)$CSQ, function(x) x[1])),]
}
